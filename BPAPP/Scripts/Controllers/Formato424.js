...
$(document).ready(function () {
    ...
    tablaformato424 = $('#tbnivel').DataTable({
        "ajax": {
            "url": $.MisUrls.url.UrlGetFormato424,
            "type": "GET",
            "datatype": "json"
        },
        "columns": [
...
function eliminar($idnivel) {
    if (confirm("¿Desea eliminar el nivel academico seleccionado?")) {
        jQuery.ajax({
            url: $.MisUrls.url.UrlDeleteFormato424 + "?idnivel=" + $idnivel,
            type: "GET",
            dataType: "json",
            contentType: "application/json; charset=utf-8",
            success: function (data) {
...
function Guardar() {
    ...
    if ($("#formNivel").valid()) {
        ...
        jQuery.ajax({
            url: $.MisUrls.url.UrlPostFormato424,
            type: "POST",
